...
(define (mod-exp b e m)
  (cond [(eq? e 0) 1]
        [(eq? (modulo e 2) 0) (modulo (expt (modulo (mod-exp b (/ e 2) m) m) 2) m)]
        [else (modulo (* (modulo (mod-exp b (- e 1) m) m) b) m)]))
...
#| Implement "encrypt" to accept a plaintext string and a public key as a list
 ...
 |     -> #t
 |   (equal? (encrypt "This should be fast!" '(1971330671 418709)) '(458148201 189194883 434312893 440095087 1505989517 440095087 189194883 1906158280 1939072533 979080909 895284233 1505989517 554357341 594947913 1505989517 1856114114 643278062 440095087 620385227 1686440477))
 |     -> #t
 |#
...
(define (encrypt plaintext key)
  (get_crypt (string->list plaintext) key))

(define (get_crypt charList key)
  (cond [(eq? charList '()) '()]
        [else (cons (mod-exp (char->integer (car charList)) (car (cdr key)) (car key)) (get_crypt (cdr charList) key))]))
...
#| Implement "decrypt" to accept a ciphertext (list of naturals) and a private key as a list
 ...
 |
 | All test cases for this function should finish near-instantaneously.  If your implementation
 |  is taking forever to finish, you most likely haven't correctly implemented the efficient algorithm
 |  for mod-exp.
 |
 | Helpful EOPL functions:
 ...
 |     -> "RACKET IS GREAT"
 |	 (decrypt '(458148201 189194883 434312893 440095087 1505989517 440095087 189194883 1906158280 1939072533 979080909 895284233 1505989517 554357341 594947913 1505989517 1856114114 643278062 440095087 620385227 1686440477) '(1971330671 82301189))
 |     -> "This should be fast!"
 |#
...
(define (decrypt ciphertext key)
  (list->string (de_crypt ciphertext key)))

(define (de_crypt intList key)
  (cond [(eq? intList '()) '()]
        [else (cons (integer->char (mod-exp (car intList) (car (cdr key)) (car key))) (de_crypt (cdr intList) key))]))
