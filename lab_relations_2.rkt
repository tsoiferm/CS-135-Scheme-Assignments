...
;; 5 PTS
(define (compose S R)
  (make-set (compose_help S R)))

(define (compose_help S R)
  (cond [(or (null? R) (null? S)) '()]
        [(null? (cdr R)) (make_comp (car (car R)) (relate (car (cdr (car R))) S) )]
        [else (append (make_comp (car (car R)) (relate (car (cdr (car R))) S)) (compose S (cdr R)) )]))

(define (make_comp e R)
  (cond [(null? R) '()]
        [else (cons (append (list e) (list (car R))) (make_comp e (cdr R)))]))
...
(define (transitive-closure R)
  (cond [(eq? R '()) '()]
        [else (help (cardinality R) R)]))

(define (help e R)
  (cond [(eq? e 1) (power e R)]
        [else (union (power e R) (help (- e 1) R))]))
...
;; 3 PTS
(define (transitive? R)
  (cond [(equal? R (transitive-closure R)) #t]
        [else #f]))
...
(define (equivalence? n R)
  (cond [(and (reflexive? n R)(and (symmetric? R) (transitive? R))) #t]
        [else #f]))
...
(define (involutive? n R)
  (cond [(set-equal? (compose R R) R) #t]
        [else #f])) 
