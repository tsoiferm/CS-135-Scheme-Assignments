...
(define (get-vertices G)
 (usort (help-get-vert G)))

(define (help-get-vert G)
   (cond [(eq? G '()) '()]
       [(or (eq? (cdr G) '())) (car G)]
       [(append (car G)(help-get-vert (cdr G)))]))
...
(define (degree G v)
  (cond [(eq? G '()) 0]
        ; [(eq? (cdr G) '()) 0]
        [(and (eq? (car (car G)) v) (eq? (car (cdr (car G))) v)) (+ 2 (degree (cdr G) v))]
        [(or (eq? (car (car G)) v) (eq? (car (cdr (car G))) v)) (+ 1 (degree (cdr G) v))]
...
;; 3 PTS
(define (all-even? G)
  (define (helper-all-even v)
    (degree G v))
  (helper (map even? (map helper-all-even (get-vertices G)))) )
...
;; 4 PTS
(define (adjacent G v)
  (usort (adj-helper G v)))

(define (adj-helper G v)
  (cond [(eq? G '()) '()]
        [(and (eq? (car (car G)) v) (eq? (car (cdr (car G))) v)) (append (list v) (adj-helper (cdr G) v))]
        [(eq? (car (car G)) v) (append (cdr (car G)) (adj-helper (cdr G) v))]
        [(eq? (car (cdr (car G))) v) (append (list (car (car G))) (adj-helper (cdr G) v))]
        [else (append '() (adj-helper (cdr G) v))])) 
...
;; 4 PTS
(define (connected? G)
  (define (helper-connected v)
    (dfs G v))
  (define (connect-help2 L)
    (connect-help1 G L))
  (helper-con (map connect-help2 (map usort (map helper-connected (get-vertices G))))))

(define (connect-help1 G L)
  (eq? (car L) (get-vertices G)))

(define (helper-con L)
  (cond [(eq? L '()) #t]
        [(eq? (car L) #f) #f]
        [else (helper-con (cdr L))]))
