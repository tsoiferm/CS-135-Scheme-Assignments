...
(define (reflexive? n R)
  (cond [(eq? n 0) #t]
        [(eq? (element? R (list n n)) #f) #f]
        [else (reflexive? (- n 1) R)]))
...
(define (relate x R)
  (cond [(eq? R '()) '()]
        [(eq? (car (car R)) x) (append (cdr (car R)) (relate x (cdr R)))]
        [else (relate x (cdr R))]))
...
(define (getList R)
    (cond [(eq? R '()) '()]
          [else (append (cdr (car R)) (getList (cdr R)))]))

(define (injective? R)
  (cond [(equal? (getList R) (make-set (getList R))) #t]
        [else #f]))
